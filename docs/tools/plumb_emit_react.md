...
- Gradients, multi-layer shadows, opacity, backdrop-filter, border radius (including pill/circle), and typography (align/decoration/case/font-family/letter-spacing/line-height) all round-trip into inline styles. `fontFamily` is web-adapter-only today (Figma-sourced text has no equivalent field yet); pair it with `plumb_import_web`'s top-level `fontLinks` so the referenced font actually loads instead of silently falling back.
