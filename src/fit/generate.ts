...
- Images / icons / logos are ASSETS — use the real thing, never redraw them as CSS. A node with \`assetId\` is a raster export: render an <img> pointing at the exported file and stamp data-plumb-asset="<assetId>" on it. A node with \`vectorPath\` (or type "vector") is a vector: render an inline <svg> using the provided path \`d\` string(s) — do NOT approximate a logo/glyph with CSS borders, gradients, or shapes. Only when no asset/vector/path is available may you fall back to a correctly-sized neutral placeholder (still tagged with data-plumb-id). Verify flags a visual node rendered without real image/vector content as an error, so a redrawn logo will tank the score — it should.
